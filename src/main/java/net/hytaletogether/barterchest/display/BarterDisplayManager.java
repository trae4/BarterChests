package net.hytaletogether.barterchest.display;
...
public class BarterDisplayManager {
    ...
    /**
     * Refresh all shop displays across all worlds.
     * Called periodically to restore displays removed by chunk cleanup.
     */
    public static void refreshAllDisplays() {
        LOGGER.at(Level.INFO).log("Starting periodic display refresh...");
        
        Map<String, Set<Long>> shopsByWorld = ShopLocationRegistry.getSnapshot();
        int refreshed = 0;
        int total = 0;
        
        for (Map.Entry<String, Set<Long>> entry : shopsByWorld.entrySet()) {
            ...
            for (long packed : positions) {
                int x = ShopLocationRegistry.unpackX(packed);
                int y = ShopLocationRegistry.unpackY(packed);
                int z = ShopLocationRegistry.unpackZ(packed);
                total++;
                ...
                world.execute(() -> {
                    try {
                        refreshDisplayAt(world, bx, by, bz);
                    } catch (Exception e) {
                        LOGGER.at(Level.WARNING).log("Failed to refresh display at %d,%d,%d: %s", 
        ...
        LOGGER.at(Level.INFO).log("Display refresh queued for %d/%d shops", refreshed, total);
    }
    
    /**
     * Refresh display for a single shop location.
     * Must be called on the world thread.
     */
    public static void refreshDisplayAt(@Nonnull World world, int blockX, int blockY, int blockZ) {
        // Get the block state at this location
        Object blockState = world.getState(blockX, blockY, blockZ, false);
        if (!(blockState instanceof BarterChestBlockState)) {
            return;
        }
        
        BarterChestBlockState shop = (BarterChestBlockState) blockState;
        
        // Check if display entity still exists
        ...
        if (displayUUID != null) {
            ...
            if (entityRef != null && entityRef.isValid()) {
                // Display still exists, no need to refresh
                return;
            }
            
            // Entity is gone, clear the stale UUID
            shop.setDisplayEntityUUID(null);
        }
        
        // Recreate the display
        createOrUpdateDisplay(shop, world, blockX, blockY, blockZ);
    }

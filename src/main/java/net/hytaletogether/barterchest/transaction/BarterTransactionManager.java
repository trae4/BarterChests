package net.hytaletogether.barterchest.transaction;
 ...
 * 
 * Buy: Customer gives currency items (or digital currency), receives shop items
 * Sell: Customer gives items, receives currency items (or digital currency) from shop
 * 
 * When digital currency is used:
 * - Buy: Withdrawn from customer's wallet, deposited to shop owner's wallet
 * - Sell: Withdrawn from shop owner's wallet, deposited to customer's wallet
 */
public class BarterTransactionManager {
     ...
     * @param customerUUID The customer's UUID (for digital currency)
    ...
    public static TransactionResult buyFromShop(
        ...
        boolean isDigitalCurrency = EconomyIntegration.isDigitalCurrency(currencyItemId);

        // IMPORTANT:
        // buyPrice/sellPrice are defined per "transaction" (quantityPerTransaction items).
        // Example: qtyPerTx=10 and buyPrice=2 means "10 items for 2 currency".
        // The UI buttons always request quantities that are multiples of qtyPerTx.
        int qtyPerTx = Math.max(1, listing.getQuantityPerTransaction());
        if (quantity % qtyPerTx != 0) {
            return TransactionResult.failure(TransactionResult.Status.INVALID_QUANTITY,
                "Quantity must be a multiple of " + qtyPerTx + ".");
        }

        int pricePerTransaction = listing.getBuyPrice();
        int transactions = quantity / qtyPerTx;
        // For digital currency, prices in UI are in whole units (1 = 1 Orb)
        // but the economy API uses cents (100 = 1 Orb), so multiply by 100
        long totalCost = isDigitalCurrency ? (long) pricePerTransaction * transactions * 100L : pricePerTransaction * transactions;
        ...
        if (isDigitalCurrency) {
            // Digital currency - check wallet balance
            if (customerUUID == null) {
                return TransactionResult.failure(TransactionResult.Status.TRANSACTION_ERROR,
                    "Cannot process digital currency without player identity.");
            }
            if (!EconomyIntegration.isAvailable()) {
                return TransactionResult.failure(TransactionResult.Status.TRANSACTION_ERROR,
                    "Digital currency is not available.");
            }
            long customerBalance = EconomyIntegration.getBalance(customerUUID);
            if (customerBalance < totalCost) {
                return TransactionResult.failure(TransactionResult.Status.INSUFFICIENT_FUNDS,
                    "You need " + EconomyIntegration.formatMoney(totalCost) + " but only have " + 
                    EconomyIntegration.formatMoney(customerBalance) + ".");
            }
        } else {
            // Physical currency - check inventory
        ...
        // Check if shop has space for currency (only for physical currency)
        if (!isDigitalCurrency) {
        ...
        if (isDigitalCurrency) {
            ...
            // 1. Withdraw from customer's wallet (with notification)
            if (!EconomyIntegration.withdraw(customerUUID, totalCost, "Shop Purchase")) {
                return TransactionResult.failure(TransactionResult.Status.TRANSACTION_ERROR,
                    "Failed to process payment.");
            }
            
            // 2. Transfer items from shop to customer
            if (!transferItems(shopInventory, customerInventory, itemId, quantity)) {
                // Rollback: refund customer
                EconomyIntegration.deposit(customerUUID, totalCost, "Shop Refund");
                return TransactionResult.failure(TransactionResult.Status.TRANSACTION_ERROR,
                    "Failed to retrieve items from shop.");
            }
            
            // 3. Deposit to shop owner's wallet (with notification)
            UUID ownerUUID = shop.getOwnerUUID();
            if (ownerUUID != null) {
                EconomyIntegration.deposit(ownerUUID, totalCost, "Shop Sale");
            }
            ...
            String currencyName = EconomyIntegration.getCurrencyNamePlural();
            return TransactionResult.success(quantity,
                "Bought " + quantity + "x " + getItemName(itemId) + " for " + EconomyIntegration.formatMoney(totalCost));
        } else {
            // Physical currency transaction (original logic)
            // 1. Remove currency from customer
            if (!removeItems(customerInventory, currencyItemId, (int) totalCost)) {
                return TransactionResult.failure(TransactionResult.Status.TRANSACTION_ERROR,
                    "Failed to process payment.");
            }
            ...
            if (!transferItems(shopInventory, customerInventory, itemId, quantity)) {
                ...
                addItems(customerInventory, currencyItemId, (int) totalCost);
                return TransactionResult.failure(TransactionResult.Status.TRANSACTION_ERROR,
                    "Failed to retrieve items from shop.");
            }
            
            // 3. Add currency to shop
            if (!addItems(shopInventory, currencyItemId, (int) totalCost)) {
    ...
    /**
     * Backward-compatible overload without customerUUID.
     */
    public static TransactionResult buyFromShop(
            @Nonnull BarterChestBlockState shop,
            @Nonnull BarterListing listing,
        ...
        return buyFromShop(shop, listing, customerInventory, null, quantity);
    }
    ...
    public static TransactionResult sellToShop(
        ...
        // See buyFromShop() for pricing semantics.
        int qtyPerTx = Math.max(1, listing.getQuantityPerTransaction());
        if (quantity % qtyPerTx != 0) {
            return TransactionResult.failure(TransactionResult.Status.INVALID_QUANTITY,
                "Quantity must be a multiple of " + qtyPerTx + ".");
        ...
        int pricePerTransaction = listing.getSellPrice();
        int transactions = quantity / qtyPerTx;
        // For digital currency, prices in UI are in whole units (1 = 1 Orb)
        // but the economy API uses cents (100 = 1 Orb), so multiply by 100
        long totalPayment = isDigitalCurrency ? (long) pricePerTransaction * transactions * 100L : pricePerTransaction * transactions;
        ...
        // Check if shop owner has enough currency to pay
        UUID ownerUUID = shop.getOwnerUUID();
        if (isDigitalCurrency) {
            // Digital currency - check owner's wallet
            if (customerUUID == null) {
                return TransactionResult.failure(TransactionResult.Status.TRANSACTION_ERROR,
                    "Cannot process digital currency without player identity.");
            }
            ...
            long ownerBalance = EconomyIntegration.getBalance(ownerUUID);
            if (ownerBalance < totalPayment) {
                return TransactionResult.failure(TransactionResult.Status.INSUFFICIENT_FUNDS,
                    "Shop owner doesn't have enough " + EconomyIntegration.getCurrencyNamePlural() + " to pay you.");
            }
        } else {
            // Physical currency - check shop inventory
        ...
        // Check if customer has space for currency (only for physical currency)
        if (!isDigitalCurrency) {
        ...
        if (isDigitalCurrency) {
            // Digital currency transaction
            // 1. Withdraw from shop owner's wallet (with notification)
            if (!EconomyIntegration.withdraw(ownerUUID, totalPayment, "Shop Purchase")) {
                return TransactionResult.failure(TransactionResult.Status.TRANSACTION_ERROR,
                    "Shop owner doesn't have enough funds.");
            }
            
            // 2. Transfer items from customer to shop
            if (!transferItems(customerInventory, shopInventory, itemId, quantity)) {
                // Rollback: refund owner
                EconomyIntegration.deposit(ownerUUID, totalPayment, "Shop Refund");
                return TransactionResult.failure(TransactionResult.Status.TRANSACTION_ERROR,
                    "Failed to take your items.");
            }
            
            // 3. Deposit to customer's wallet (with notification)
            EconomyIntegration.deposit(customerUUID, totalPayment, "Shop Sale");
            
            shop.markDirty();
            
            return TransactionResult.success(quantity,
                "Sold " + quantity + "x " + getItemName(itemId) + " for " + EconomyIntegration.formatMoney(totalPayment));
        } else {
            // Physical currency transaction (original logic)
            ...
            if (!removeItems(shopInventory, currencyItemId, (int) totalPayment)) {
            ...
            if (!addItems(customerInventory, currencyItemId, (int) totalPayment)) {
    ...
    public static TransactionResult sellToShop(
        ...
        return sellToShop(shop, listing, customerInventory, null, quantity);
    }
